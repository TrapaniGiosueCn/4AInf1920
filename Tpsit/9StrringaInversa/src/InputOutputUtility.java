...
public class InputOutputUtility {
    ...
    private static InputStreamReader r = new InputStreamReader(System.in);      // System.in --> TASTIERA (in grado soltanto di acquisire)
    private static BufferedReader br = new BufferedReader(r);                   // BufferedReader (buffer = in memoria centrale, in grado di contenere) --> BACINELLA per raccogliere i dati inseriti da tastiera


    public static String leggiNome(String outStr){
        String sc=null;

        System.out.print(outStr);   // richiesta dell'input
        try {
            sc=br.readLine();       // leggere una sequenza di caratteri (sc = sequenza di caratteri) dal buffer
        } catch (IOException e) {
